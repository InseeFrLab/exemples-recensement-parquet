...
[^notealternative]: Des propositions d'enrichissements de cette documentation à partir d'implémentations alternatives, par exemple s'appuyant sur `Arrow` et `dbplyr` ou sur `Polars` sont bienvenues sur le [Github InseeFrLab/exemples-recensement-parquet].
...
Les données sont disponibles sur le site `insee.fr` aux adresses suivantes:
...
Ces fichiers doivent être téléchargés et dézippés manuellement.
Dans la suite de ce guide, il sera fait l'hypothèse 
que les données sont stockées dans un même dossier
et qu'il existe une variable _ad hoc_ enregistrant ce chemin 
dans `Python` ou `R`.

::: {.callout-caution collapse="true"}
## Chemins des données pour les exemples `Observable` 

Pour que les exemples fournis avec `Observable` par le biais
de `Quarto` soient fonctionnels, les données doivent être stockées
dans un dossier enfant du répertoire de travail. Le chemin à indiquer
est donc relatif à celui-ci. 
:::
